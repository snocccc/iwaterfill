...
<div class="bg-[#caf0f8] min-h-screen py-8">
    <div class="container mx-auto px-4">
        <h1 class="text-3xl sm:text-4xl font-bold text-[#03045e] mb-4">Purchase History</h1>
        ...
        <div class="mb-4 flex flex-col md:flex-row justify-center items-center">
            ...
            <form action="{{ route('history') }}" method="GET" class="flex items-center mb-2 md:mb-0">
            ...
            <table class="min-w-full border-collapse border border-gray-200 text-sm">
                <thead>
                    <tr class="bg-[#0077b6] text-white">
                        <th class="border border-[#90e0ef] p-2">Customer Name</th>
                        <th class="border border-[#90e0ef] p-2">Product Name</th>
                        <th class="border border-[#90e0ef] p-2">Price</th>
                        <th class="border border-[#90e0ef] p-2">Purchase Date</th>
                        <th class="border border-[#90e0ef] p-2">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($payments as $payment)
                        <tr class="border-b border-[#90e0ef] hover:bg-[#e6f8fc] transition duration-300">
                            <td class="border border-[#90e0ef] p-2">{{ $payment->username }}</td>
                            <td class="border border-[#90e0ef] p-2">{{ $payment->product_Name }}</td>
                            <td class="border border-[#90e0ef] p-2 font-medium text-[#03045e]">₱{{ number_format($payment->price, 2) }}</td>
                            <td class="border border-[#90e0ef] p-2">{{ \Carbon\Carbon::parse($payment->purchase_date)->format('F d, Y') }}</td>
                            <td class="border border-[#90e0ef] p-2">
    ...
    <script>
        $(document).ready(function() {
            ...
            // Handle date filtering directly in the frontend if needed
